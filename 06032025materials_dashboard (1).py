...
if page == "Project Description":
    ...
    if selected_use_option == "I want to use an existing analysis":
        ...
        project_final_year = st.number_input("Project timeline - final year*", value=2035, disabled=True)
    else:
        user_input = 0
    ...
    if st.button("Save", key="save_project_description"):
        track_click("Project Description", "Save")
        if project_starting_year <= project_final_year:
            ...
            st.markdown(
                f'<p style="color:red;font-weight:bold">Please enter the appropriate project timeline and try again</p>',
...
if page == "Material Supply":
    ...
    if col2.button("**Note**", key="Direct Mining"):
        track_click("Material Supply", "Direct Mining Note")
        st.markdown(
            """
    ...
    if selected_supply_option == "Manually input annual production":
        ...
        if st.button("Add", key="mine_prod1"):
            track_click("Material Supply", "Add Mine Production")
            st.session_state.mine_prod[i] = mine_prod
            st.session_state["default_input_mine"][i] = mine_prod

        mine_data = pd.DataFrame({
            "Year": years_list,
            "Annual production (tonnes)": st.session_state.mine_prod[:year_f - year_s + 1]
        })
    ...
    if rec_calculation_method == cal_methods[0]:
        ...
        if pv_material_intensity == "Constant Material Intensity":
            ...
            if st.button("Add", key="cap1"):
                track_click("Material Supply", "Add Existing PV Capacity Constant")
            ...
            mint = st.number_input(f"Enter the annual material intensity for Year {y + 1}", min_value=0.0, step=0.1, key=f"annual_mint{y}")

            if st.button("Add", key="mint1"):
                track_click("Material Supply", "Add Existing PV Capacity Variable")
                st.session_state.input["pv_capacity1"][y] = pv_capacity
        ...
        if st.button("Add", key="material supply1"):
            track_click("Material Supply", "Add Material Supply Manual")
    ...
    st.divider()
    if st.button("Save", key="save_material_supply"):
        track_click("Material Supply", "Save")
...
if page == "Material Demand":
    ...
    years_list = list(range(start_year, final_year + 1))

    if "pv_market_share" not in st.session_state or len(st.session_state.pv_market_share) != pv_years:
        st.session_state.pv_market_share = np.zeros(pv_years)
    if "pv_material_intensity" not in st.session_state or len(st.session_state.pv_material_intensity) != pv_years:
        st.session_state.pv_material_intensity = np.zeros(pv_years)
    if "pv_production" not in st.session_state or len(st.session_state.pv_production) != pv_years:
    ...
    if selected_supply_option == "Default values":
        ...
        if st.button("Add", key="pv_production1"):
            track_click("Material Demand", "Add PV Production Default")
    ...
    if selected_supply_option == "Custom User Input":
        ...
        if st.button("Add", key="pv_production2"):
            track_click("Material Demand", "Add PV Production Custom")
            st.session_state.pv_production[ind] = pv_annual_production
            st.session_state["default_input_pv_production"][ind] = pv_annual_production
            st.session_state.pv_market_share[ind] = pv_market_share
            st.session_state["default_input_pv_market_share"][ind] = pv_market_share
            st.session_state.pv_material_intensity[ind] = pv_material_intensity
            st.session_state["default_input_pv_material_intensity"][ind] = pv_material_intensity
    ...
    nonpv_demand = st.number_input("Enter the current non-PV demand for first Year (tonnes)", min_value=0.00, step=0.01, value=nonpv_demand_df)
    ...
    if st.button("Save", key="pv_demand1"):
        track_click("Material Demand", "Save")
...
if page == "Supply Gap and Price Effect":
    ...
    col2.write("")
    col2.write("")
    if col2.button("Equation", key="supply_gap_eq_btn"):
        track_click("Supply Gap and Price Effect", "Supply Gap Equation")
    ...
    if st.session_state.input["supply_option"] == 2:
        ...
        if affected == affect_choice[1]:
            ...
            if col22.button("Equation", key="dmgr_eq"):
                track_click("Supply Gap and Price Effect", "Direct Mining Equation")
                if dmgr_eq_degree == 1:
    ...
    if bp_reserve_option == 2:
        ...
        if affected2 == affect_choice2[1]:
            ...
            if col32.button("Equation", key="bpgr_eq"):
                track_click("Supply Gap and Price Effect", "Byproduction Equation")
                if bpgr_eq_degree == 1:
        ...
        if st.session_state.input["num_bp_hosts"] != 0:
            ...
            if hitchhiker1_affected == 1 or hitchhiker2_affected == 1 or hitchhiker3_affected == 1:
                ...
                if col32.button("Equation", key="bpy_eq"):
                    track_click("Supply Gap and Price Effect", "Byproduction Yield Equation")
                    if bpy_eq_degree == 1:
    ...
    st.divider()
    if st.button("Save", key="save_supply_gap"):
        track_click("Supply Gap and Price Effect", "Save")
...
if page == "Modelling Inputs Review":
    ...
    if selected_inputs == "Material Supply":
        if "save2" not in st.session_state:
            ...
            if st.session_state.input["rec_cal_method"] == 1:
                ...
                pv_df = pd.DataFrame({"Inputs": pv_inputs, "Values": pv_values})
    ...
    if selected_inputs == "Material Demand":
        if "save3" not in st.session_state:
            ...
            if st.session_state.input["pv_user_input"] == 1:
                ...
                pv_demand_lists = ["annual_incremental_capacity1", "pv_initial_incremental1", "pv_annual_deployment1", "pv_market_share", "pv_material_intensity"]
                ...
                pv_demand_df = pd.DataFrame({"Inputs": pv_demand_inputs, "Values": pv_demand_values})
    ...
    if selected_inputs == "Supply Gap and Price Effect":
        if "save4" not in st.session_state:
            ...
            sp_ef_df = pd.DataFrame({"Inputs": supply_gap_ef_inputs, "Values": supply_gap_ef_values})
            ...
            gs_df = pd.DataFrame({"Inputs": ["Global stocks"], "Values": [inputs["global_stocks"]]})
...
if page == "Calculate Results":
    ...
    if button:
        track_click("Calculate Results", "Calculate")

    _subscript_dict = {
        "PVDev": [f"Y{y}" for y in range(2023, 2051)],
        "StudyPeriod": [f"Y{y}" for y in range(2023, 2051)],
    }
    ...
    if button and "input" in st.session_state:
        ...
        for year in range(start_year, end_year + 1):
            ...
            output = model.run(params={
                ...
                "Direct Mining Supply Manual": data["mine_data"]["Annual production (tonnes)"] if isinstance(data.get("mine_data", None), pd.DataFrame) else 0,
            ...
            columns_MS = [
                "Total Annual Recycling Supply from new PV",
            ...
            columns_MP = [
                "Material in the market Cumulative",
                "Annual Material Surplus",
                "Material Price",
                "PV Technology material intensity"
            ]

            row_idx = year - start_year + 2023
            MS[ind, :-1] = output.loc[row_idx, columns_MS].values
            MS[ind, -1] = np.sum(MS[ind, :-1])

            MD[ind, :-1] = output.loc[row_idx, columns_MD].values
            MD[ind, -1] = np.sum(MD[ind, 2:4])

            MP[ind, :] = output.loc[row_idx, columns_MP].values
    ...
    if st.session_state["Cal"] == 1:
        ...
        if button2:
            track_click("Calculate Results", "Show Material Supply")
        ...
        if button3:
            track_click("Calculate Results", "Show Material Demand")
        ...
        if button4:
            track_click("Calculate Results", "Show Material Price and Availability")
        ...
        if button5:
            track_click("Calculate Results", "Download Raw Data")
            title = st.session_state.input["project_title"]
            output = st.session_state.output["output"]
            name = f'raw_data_{title}.csv'
            csv_data = output.to_csv(index=True)
            st.download_button(label="Download data as CSV", data=csv_data, file_name=name, mime='text/csv')
...
if page == "Plot Results":
    ...
    if op1:
        track_click("Plot Results", "Open Material Supply Plot")
        ...
        fig1 = px.line(data1, x=data1.index, y=selected_metric, labels={"x": "Year", "y": "Value"})
    ...
    if op2:
        track_click("Plot Results", "Open Material Demand Plot")
        ...
        fig2 = px.line(data2, x=data2.index, y=selected_metric, labels={"x": "Year", "y": "Value"})
    ...
    if op3:
        track_click("Plot Results", "Open Material Price Plot")
        ...
        fig3 = px.line(data3, x=data3.index, y=selected_metric, labels={"x": "Year", "y": "Value"})
