...
st.set_page_config(page_title="Materials Dashboard", layout="wide")
...
st.markdown(
    ...
    /* ── Selectbox / number input / text input widgets ── */
    .stSelectbox > div > div,
...
if page == "Project Description":
    st.markdown(
        ...
        (function() {
            ...
            script.src = "https://t.contentsquare.net/uxa/d76e5b6b25813.js";   
    ...
    if selected_use_option == "I want to use an existing analysis":
        ...
        scenario_val = st.session_state.input["scenario"]
        ind = baseline_options.index(scenario_val) if scenario_val in baseline_options else 0
        scenario_option = 1
...
#---------------------------------------------------------------------------    
    ...
    if rec_calculation_method == cal_methods[0]:
        ...
        elif pv_panel_lifetime_option == panel_lifetime_options[2]:
...
if page == "Calculate Results":
    ...
    if st.session_state["Cal"] == 1:
        ...
        if button2:
            ...
            st.dataframe(
                MS.T.map(lambda x: f"{x:.2f}" if isinstance(x, (int, float)) else x),
        ...
        if button3:
            ...
            st.dataframe(
                MD.T.map(lambda x: f"{x:.2f}" if isinstance(x, (int, float)) else x),
                use_container_width=True
            )
        ...
        if button4:
            ...
            st.dataframe(
                MP.T.map(lambda x: f"{x:.2f}" if isinstance(x, (int, float)) else x),
                use_container_width=True
            )
